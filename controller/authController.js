...
router.post('/register',(req,res) => {
     ...
     User.create({
          ...
          if(err) return res.status(500).send('Error while Registration')
...
router.post('/login',(req,res) => {
     User.findOne({email:req.body.email},(err,user) => {
          if(err) return res.status(500).send({auth:false,token:'Error while Registration'})
...
router.get('/userinfo',(req,res) => {
     let token = req.headers['your-access-token'];
